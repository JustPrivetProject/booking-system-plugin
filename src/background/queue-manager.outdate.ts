...
import { generateUniqueId, consoleLog, consoleError, consoleLogWithoutSave } from '../utils/index';
...
class QueueManager {
    ...
    async addToQueue(newItem: RetryObject) {
        return this._synchronizedOperation(async () => {
            const result = (await chrome.storage.local.get(this.storageKey)) as {
                [key: string]: RetryObject[];
            };
            ...
            if (status === 'success') {
                const hasSameTvAppId = queue.some(item => item.tvAppId === tvAppId);
                if (!hasSameTvAppId) {
                    consoleLog(
            ...
            const isDuplicate = queue.some(
                item => item.tvAppId === tvAppId && item.startSlot === startSlot,
            );
    ...
    async _synchronizedOperation(operation) {
        ...
        try {
            ...
            consoleError(`Error in synchronized operation for ${this.storageKey}:`, error);
            throw error;
        } finally {
            // Release the lock
            this.mutex = false;
                ...
                for (const req of inProgressRequests) {
                    try {
                        ...
                        await this.updateQueueItem(req.id, {
                            status: 'error',
                            status_message:
                                error instanceof Error ? error.message : 'Error on processing',
                        });
                    }
                }
            } catch (error) {
