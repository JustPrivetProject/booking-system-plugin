...
export async function showSessionExpireModal(opts?: {
    ...
    function waitForOneMinute() {
        ...
        if (desc) {
            desc.innerHTML = `Odświeżenie strony zostanie wykonane, gdy czas sesji osiągnie 1 minutę.<br><button id="close-modal-btn" style="margin-top:18px;padding:8px 24px;border-radius:4px;background:var(--color-primary,#00aacc);color:#fff;border:none;cursor:pointer;">Zamknij</button>`
        }
        ...
        // Следим за sessionTime
        let checkMinute: number | undefined = window.setInterval(() => {
            const timerSpanSession = document.getElementById('sessionTime')
            if (timerSpanSession && timerSpanSession.textContent) {
                const [minStr, secStr] = timerSpanSession.textContent.split(':')
                ...
                if (
                    ...
                    if (checkMinute !== undefined) clearInterval(checkMinute)
                    if (autoCloseTimeout !== undefined)
                        clearTimeout(autoCloseTimeout)
                    startCountdown()
                }
