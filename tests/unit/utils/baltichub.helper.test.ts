    ...
    describe('handleErrorResponse', () => {
        ...
        it('should handle TaskWasUsedInAnotherTva error', () => {
            ...
            expect(result.status_message).toBe(
                'Zadanie zakończone w innym wątku'
            )
        })
        ...
        it('should handle ToMuchTransactionInSector error', () => {
            ...
            expect(result.status_message).toBe(
                'Za duża ilość transakcji w sektorze'
            )
        ...
        it('should handle HTML error pages', () => {
            ...
            const parsedResponse =
                '<!DOCTYPE html><html><h1>Error 500</h1></html>'
            const result = handleErrorResponse(
                mockReq,
                parsedResponse,
                'tv-app-123',
        ...
        it('should handle Error 404 in HTML', () => {
            const parsedResponse =
                '<!DOCTYPE html><html><h1>Error 404</h1></html>'
            const result = handleErrorResponse(
                mockReq,
                parsedResponse,
                'tv-app-123',
        ...
        it('should handle Error 403 in HTML', () => {
            const parsedResponse =
                '<!DOCTYPE html><html><h1>Error 403</h1></html>'
            const result = handleErrorResponse(
                mockReq,
                parsedResponse,
                'tv-app-123',
        ...
        it('should handle Error 401 in HTML', () => {
            const parsedResponse =
                '<!DOCTYPE html><html><h1>Error 401</h1></html>'
            const result = handleErrorResponse(
                mockReq,
                parsedResponse,
                'tv-app-123',
        ...
        it('should handle Error 400 in HTML', () => {
            const parsedResponse =
                '<!DOCTYPE html><html><h1>Error 400</h1></html>'
            const result = handleErrorResponse(
                mockReq,
                parsedResponse,
                'tv-app-123',
        ...
        it('should handle non-JSON, non-HTML errors', () => {
            ...
            expect(result.status).toBe(Statuses.ERROR)
            expect(result.status_message).toBe(
                'Nieznany błąd (niepoprawny format odpowiedzi)'
            )
        })

        it('should extract additional error details from HTML', () => {
            const parsedResponse =
                '<!DOCTYPE html><html><h1>Custom Error Message</h1></html>'
            const result = handleErrorResponse(
                mockReq,
                parsedResponse,
                'tv-app-123',
