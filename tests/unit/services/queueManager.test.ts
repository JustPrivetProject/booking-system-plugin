...
describe('QueueManager', () => {
    ...
    describe('removeFromQueue', () => {
        it('should remove item by id', async () => {
            ...
            expect(result).toHaveLength(0)
            expect(mockEvents.onItemRemoved).toHaveBeenCalledWith(
                mockRetryObject.id
            )
    ...
    describe('updateQueueItem', () => {
        it('should update existing item', async () => {
            ...
            const result = await queueManager.updateQueueItem(
                mockRetryObject.id,
                updates
            )
            ...
            expect(mockEvents.onItemUpdated).toHaveBeenCalledWith(
                mockRetryObject.id,
                updates
            )
        })
    ...
    describe('startProcessing', () => {
        ...
        it('should call onProcessingError for cycle-level errors', async () => {
            ...
            expect(mockEvents.onProcessingError).toHaveBeenCalledWith(
                expect.any(Error)
            )
        })
    })
